<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" element-loading-text="拼命加载中"  :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" v-if="dialogFormVisible"  size="full">
      <el-form :model="temp" ref="temp" :rules="rules" label-width="100px">
        ...
        <el-form-item label="权限菜单" >
          <el-tree :data="permsDatas" :props="defaultProps" ref="tree" node-key="id" show-checkbox highlight-current></el-tree>
        </el-form-item>
      </el-form>
      ...
      handleUpdate(row) {
        ...
        this.$nextTick(() => {
          getEidtRole(row.id).then(response => {
            ...
            this.$refs.tree.setCheckedKeys(datas.myrole)
          })
        })
      },
      resetTemp() {
