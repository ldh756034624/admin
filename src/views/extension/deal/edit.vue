  ...
  <div class="app-container">
    ...
    <el-tabs type="border-card" v-model="labelName">
      ...
      <el-tab-pane label="还款计划" name="还款计划">
        <tab-plan :business-info="businessInfo"></tab-plan>
      </el-tab-pane> -->
      <el-tab-pane label="合同借据" name="合同借据">
        <detail-file-list :id="6" :fileslist="businessInfo.protocolFile.filelist" name="原合同数据"></detail-file-list>
        <add-file-list :id="10" ref="add_file" :fileslist="fileList" name="展期合同数据"></add-file-list>
      </el-tab-pane>
        ...
        <table class="table table-bordered table-info2">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td></td>
              <td>
                <el-select v-model="planForm.assign_user_id">
                  <el-option v-for="item in clientList" :value="item.id" :label="item.name" :key="item.id">{{item.name}}</el-option>
                </el-select>
              </td>
              <td></td>
            </tr>
...
import TabExtension from '@/components/Info/tabExtension'
import { getDealDetail, saveDealDetail } from '@/api/extension';
...
export default {
  components: { AddFileList, DetailFileList, Sticky, TabCustomer, TabSheet, TabBusiness, TabAssure, TabBgsurveyExtension, TabContract, TabPlanExtension, TabProgress, TabExtension, TabGrant },
  props: {
    id: {
      type: Number
    },
  ...
  methods: {
    getInit() {
      getDealDetail(this.id).then(response => {
        const datas = response.data;
        this.list = datas.periodProgress;
        this.businessInfo = datas;
        this.clientList = datas.checkUserList;
      })
    },
    getCustomerDetail() {
    ...
    handleSave() {
      if (!this.planForm.assign_user_id) {
        this.$message({ message: '请选择指派人' });
        return;
      }
      const fileId = this.$refs['add_file'].file_id;
      const fileTypeId = this.$refs['add_file'].file_type_id;

      for (let i = 0, len = fileId.length; i < len; i++) {
        fileId[i] ? this.planForm.file_id.push(fileId[i]) : '';    // 筛选数值中为空或者是undefined的值
      }
      for (let i = 0, len = fileTypeId.length; i < len; i++) {
        fileTypeId[i] ? this.planForm.file_type.push(fileTypeId[i]) : ''; // 筛选数值中为空或者是undefined的值
      }
      saveDealDetail(this.planForm, this.id).then(response => {
