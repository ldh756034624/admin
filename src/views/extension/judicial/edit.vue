...
import { getJudicailDetail, saveJudicailDetail } from '@/api/extension';
...
export default {
  ...
  methods: {
    getInit() {
      getJudicailDetail(this.id).then(response => {
    ...
    handleSave() {
      saveJudicailDetail(this.dataForm, this.id).then(response => {
