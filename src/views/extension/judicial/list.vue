  ...
  <div class="app-container">
    ...
    <el-table :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="业务编号">
        <template scope="scope">
          <span>{{scope.row.repayment.loan.code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="还款计划编号">
        <template scope="scope">
          <span>{{scope.row.repayment.code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="业务品种">
        <template scope="scope">
          <span>{{scope.row.repayment.loan.businessTypeName }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="客户名称">
        <template scope="scope">
          <span>{{scope.row.repayment.loan.companyName}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="贷款到期日">
        <template scope="scope">
          <span>{{scope.row.repayment.date}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="应还本金">
        <template scope="scope">
          <span>{{scope.row.money}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="应还利息">
        <template scope="scope">
          <span>{{scope.row.repayment.interest}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="展期应还本金">
        <template scope="scope">
          <span>{{scope.row.money}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="展期应还利息">
        <template scope="scope">
      ...
      <el-table-column align="center" label="操作">
        <template scope="scope">
          <el-button size="small" type="success" icon="eidt" @click="handleEdit(scope.row)">审查</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹出窗口 -->
    <el-dialog title="审查" :visible.sync="dialogFormVisible" v-if="dialogFormVisible"  size="full">
...
<script>
  import { getJudicailList } from '@/api/extension';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getJudicailList(this.listQuery).then(response => {
